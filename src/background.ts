 ...
 */
chrome.runtime.onMessage.addListener((event: TExensionEvent, sender, response) => {
  if (['content', 'popup'].indexOf(event.from) !== -1) {
    // Incoming message to handle
    console.log('<i> Incoming event:', { event });
    switch (event.action) {
      case 'OPEN_PRIVATE_LINK': {
        void chrome.windows.create({ url: event.data.url, incognito: true });
        response(true);
        break;
      }
      default:
